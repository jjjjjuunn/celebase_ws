...
# node:20 컨테이너로 `tsx seeds/run.ts` 실행(DB password 는 db 컨테이너 env 에서 런타임에
# 읽음 — 스크립트에 시크릿 하드코딩 없음). seed loader 는 멱등 + 트랜잭션이라 재실행/실패 안전.
#
...
set -euo pipefail
cd /app
rm -rf /tmp/celebase-seed-run && mkdir -p /tmp/celebase-seed-run
tar xzf /tmp/celebase-staging-seed.tar.gz -C /tmp/celebase-seed-run 2>/dev/null
PW=$(sudo docker compose exec -T db printenv POSTGRES_PASSWORD | tr -d '\r\n')
sudo docker run --rm --network app_default \
  -v /tmp/celebase-seed-run/db:/db -w /db \
  -e DATABASE_URL="postgresql://celebbase:${PW}@db:5432/celebbase" \
  node:20 sh -c 'npm install --no-audit --no-fund --silent pg tsx >/dev/null 2>&1 && npx tsx seeds/run.ts'
sudo rm -rf /tmp/celebase-seed-run /tmp/celebase-staging-seed.tar.gz
REMOTE
