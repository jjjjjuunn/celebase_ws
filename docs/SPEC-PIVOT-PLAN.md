...
| IMPL-MOBILE-SUB-SCHEMA-001 | 🟡 in-progress (PR pending) | §4.2 Subscriptions Internal (wire shape `{tier}` 로 simplify, BFF parse 정렬), §11 BFF layer (GET /api/subscriptions/me 단락) | SPEC-SYNC-DEFER (`pipeline/runs/IMPL-MOBILE-SUB-SCHEMA-001/SPEC-SYNC-DEFER.md`) — staging hotfix 우선 머지, 본문 backfill 은 §3 `SPEC-SYNC-SUB-SCHEMA-001` 별도 |
...
| 🟡 `SPEC-SYNC-SUB-SCHEMA-001` | IMPL-MOBILE-SUB-SCHEMA-001 (이 PR) | §4.2 Subscriptions Internal — wire shape `{tier}` simplify + commerce-service 향후 full-detail endpoint 분리 의도 명시. §11 BFF GET /api/subscriptions/me 단락 동일 정렬 |
