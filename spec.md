...
**Staging deployment infrastructure (CHORE-MOBILE-STAGING-BFF-001)**: `apps/web` BFF 와 BE service 5개 (`user-service`, `content-service`, `commerce-service`, `meal-plan-engine`, `analytics-service`) 는 staging EC2 (Elastic IP) 위에서 docker-compose 로 운영된다. public ingress 는 단일 hostname (`<STAGING_DOMAIN>`, Route53 hosted zone) 에서 **Caddy** 가 TLS 종료 (Let's Encrypt HTTP-01 자동 발급/갱신, `caddy_data` named volume 으로 cert 영속화) + 두 layer 라우팅 수행:
