...
- **결정(3-way 리뷰 반영)**: A안 = celebase.app을 기존 staging 박스(`184.32.66.152`)에 연결하되 **apex는 랜딩+legal 전용으로 스코프**. 증거: 모바일 실제 API base는 `apps/mobile/.env`상 `staging.celebase.app`(코드의 celebase.app은 주석 힌트일 뿐). #203 랜딩은 완전 정적(`landing/data.ts` static literal, 런타임 `/api` 호출 없음) → apex `/api` 차단 안전.
