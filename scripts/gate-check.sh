...
check_fe_slice_smoke() {
  ...
  if [[ "$ready" != "true" ]]; then
    ...
    for route in /slice /slice/tokens /slice/primitives /slice/components /slice/composites; do
      local code
      code=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:${port}${route}" 2>/dev/null || echo "000")
      # /slice, /slice/tokens, /slice/primitives, /slice/composites: 200 required
      # (primitives required since IMPL-UI-002 G3-b; composites required since IMPL-UI-003 G5-b showcase landed).
      # /slice/components: legacy alias, 200 OR 404 allowed.
      if [[ "$route" == "/slice" || "$route" == "/slice/tokens" || "$route" == "/slice/primitives" || "$route" == "/slice/composites" ]]; then
        if [[ "$code" != "200" ]]; then
