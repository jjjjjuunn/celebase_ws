...
export const GET = createProtectedRoute(async (req: NextRequest, session: Session) => {
  ...
  const result = await fetchBff('meal-plan', `/meal-plans${search}`, {
    method: 'GET',
...
export const POST = createProtectedRoute(async (req: NextRequest, session: Session) => {
  ...
  const parsed = schemas.GenerateMealPlanRequestSchema.safeParse(body);
  if (!parsed.success) {
    return new Response(
  ...
  const forwardedFor = req.headers.get('x-forwarded-for') ?? undefined;
  const result = await fetchBff('meal-plan', '/meal-plans/generate', {
    method: 'POST',
