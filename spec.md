...
CREATE TABLE bio_profiles (
    ...
    */
    ...
    -- P1 (Migration 0021, IMPL-MEAL-P1-PROFILE-SCHEMA-001)
    exercise_sessions JSONB NOT NULL DEFAULT '[]'::jsonb,
    /*
      exercise_sessions schema (P1-D Exercise EE Model B 입력 baseline):
      [
        { "type": "running", "intensity": "moderate",
          "duration_min": 45, "frequency_per_week": 3 }
      ]
      type/intensity 는 free string. P1-D 가 MET table 매핑.
    */
    goal_pace       VARCHAR(20) NOT NULL DEFAULT 'moderate'
                    CHECK (goal_pace IN ('slow','moderate','aggressive')),
    /*
      goal_pace — weight_loss/muscle_gain 의 사용자 선택 속도 (P1-C calorie_adjuster 분기).
      moderate 는 기존 GOAL_FACTORS 와 일치 (backward-compat).
      maintenance/glp1_support/athletic_performance 는 goal_pace 무시.
    */
...
**Step 1: Calorie Adjustment** (IMPL-MEAL-P1-GOAL-PACE-001)
```
target_kcal = TDEE × goal_factor

# weight_loss / muscle_gain: bio_profiles.goal_pace 분기 우선
goal_factor (GOAL_PACE_MULTIPLIERS):
  weight_loss × slow       → 0.90 (10% deficit, beginner-safe)
  weight_loss × moderate   → 0.80 (20% deficit, default)
  weight_loss × aggressive → 0.75 (25% deficit, Aragon 2017 상한)
  muscle_gain × slow       → 1.05 (5% surplus, Slater 2019 권장 하한)
  muscle_gain × moderate   → 1.15 (15% surplus, default)
  muscle_gain × aggressive → 1.20 (20% surplus, 보수적 default; 25% unlock 은 CHORE-MEAL-AGGRESSIVE-PROTEIN-SAFEGUARDS 후속)

# 그 외 primary_goal 은 goal_pace 무시 (기존 GOAL_FACTORS)
goal_factor (기존):
  maintenance       → 1.00
  glp1_support      → 0.90 (muscle-sparing moderate deficit)
  athletic_performance → 1.10–1.25 (activity-dependent, very_active → 1.25)

# NUTRITION_BOUNDS clamp [1200, 5000] 적용 후 반환
```
...
- Mifflin/Frankenfield consensus: 25%+ deficit 은 LBM loss + adaptive thermogenesis 위험
...
**Step 5: ILP Constraint Solver** (IMPL-MEAL-P0-ILP-001-a/b, PR-D)

`services/meal-plan-engine/src/engine/plan_solver.py` 가 OR-Tools CP-SAT 솔버로 7일 식단 조합을 **결정성** 최적화.

```python
# 결정변수: x[d, mt, i] ∈ {0,1} — day d, meal_type mt, recipe i 선택 여부
# 하드 제약 (6):
#   1. 슬롯당 정확히 1 recipe (Σ_i x[d, mt, i] = 1)
#   2. forbidden_ids 차단 (x[d, mt, i] = 0 if recipe_i ∈ forbidden)
#   3. max_repeats ≤ 2 (7-day window 내 동일 recipe 2회 초과 금지)
#   4. 일일 kcal band: target ± 100 kcal
#   5. 단백질 ≥ 95% × required (calorie_adjuster 결과)
#   6. fat_ratio ∈ [0.20, 0.35] (선형화: total_fat_kcal × 100 ≥/≤ ratio × total_cal)
# 목적함수 (4 term):
#   λ_kcal · |kcal_gap| + λ_protein · protein_shortfall +
#   λ_macro · fat_band + λ_variety · repeats
# 가중치 default: kcal=1.0, protein=0.5, macro=0.3, variety=0.2 (config override)
# 결정성: random_seed=42 + num_search_workers=1 + interleave_search=False
```
...
- 결정성: `rng = random.Random(settings.ILP_RANDOM_SEED)` 명시 주입 (P1 LESSONS 패턴)
- 사용 조건: ILPTimeoutError / ILPInfeasibleError / ILPModelError, 또는 `PIPELINE_USE_ILP=False`

**NUTRITION_BOUNDS defensive validation**: `plan_solver` 진입 시 target_kcal ∈ [1200, 5000] 검증 (caller calorie_adjuster 가 이미 clamp하지만 standalone 호출 시 안전망).

### 5.4 Model Serving

| Component | Tech |
|-----------|------|
| **Plan Solver (P0.4)** | **OR-Tools CP-SAT (`ortools==9.15.*`)** — 7일 식단 결정성 ILP, seed=42 + workers=1 + interleave_search=False |
| Substitution Engine | Rule-based + Embedding similarity (sentence-transformers) |
| Nutrition Optimizer | Linear Programming (scipy.optimize) — legacy, ILP solver 로 대체 진행 |
...
|------|----------|-------------------|
| 칼로리 계산 | weight_kg, height_cm, activity_level, primary_goal, **goal_pace** (P1-C), tdee_kcal, target_kcal | biomarkers 전체, medications, medical_conditions |
| 매크로 재분배 | weight_kg, activity_level, diet_type, medications (GLP-1 force) | 혈액 검사 결과, 알레르기 상세 |
...
def minimize_profile(full_profile: BioProfile, task: PipelineTask) -> dict:
    ...
    TASK_FIELD_MAP = {
        'calorie_adjustment': [
            ...
            'goal_pace',  # P1-C (IMPL-MEAL-P1-GOAL-PACE-001)
...
### 5.9 BMR Dispatch (IMPL-MEAL-P1-BMR-DISPATCH-001)

`services/user-service/src/services/bio-profile.service.ts` `calcBmr` 가 `body_fat_pct` 기반 분기:

```ts
// body_fat_pct ∈ [3, 60] (DB CHECK 와 일치) → Katch-McArdle 우선
// 외 → Mifflin-St Jeor fallback (회귀 보호)

if (body_fat_pct !== null && body_fat_pct >= 3 && body_fat_pct <= 60) {
  const lbm = weight_kg * (1 - body_fat_pct / 100);
  return Math.round(370 + 21.6 * lbm);    // Katch-McArdle (1973)
}
...
**근거 출처**:
- Mifflin MD et al., Am J Clin Nutr 1990;51(2):241-7. PubMed 2305711 — 일반인구 표준 추정.
- Katch FI, McArdle WD, 1973; McArdle WD et al., Med Sci Sports Exerc 1971;3(4):182-6 — LBM 기반 더 직접적 metabolic predictor.
- Frankenfield DC et al., J Am Diet Assoc 2005;105(5):775-89 — 일반 인구 대상 Mifflin 가장 robust (= fallback 정당화).

**수치 예시** (90kg male, 178cm, 36yo):
- body_fat null → Mifflin 1837
- body_fat 15% (LBM 76.5) → Katch 2022 (muscular → 더 높은 BMR)
- body_fat 30% (LBM 63) → Katch 1731 (obese → 더 낮은 BMR)

**fallback**: weight_kg null → `BMR_SAFE_FALLBACK_KCAL = 1800`. body_fat_pct 신뢰도 (DEXA > BIA > visual)는 P2 source flag chore 위임.

### 5.10 Source of Truth 정책 (CHORE-MEAL-TARGET-KCAL-SOT-001)

cross-service nutrition 계산의 single source 분리:

| 필드 | SoT | 출처 모듈 |
|------|-----|-----------|
| `bmr_kcal` | user-service | `bio-profile.service.ts` `calcBmr` (§5.9) |
| `tdee_kcal` | user-service | `bmr × ACTIVITY_MULTIPLIERS` |
| `target_kcal` | **meal-plan-engine** | `calorie_adjuster.adjust_calories` (§5.3 Step 1, goal_pace 분기) |
| `macro_targets` | **meal-plan-engine** | `macro_rebalancer.rebalance_macros` (AMDR + medications + diet_type) |
...
**현재 transition 상태**: user-service `recalculate` 가 `target_kcal = tdee - 500` (절대 deficit), `calcMacroTargets` 25/50/25 hardcode 로 estimate 저장 — engine 결과와 다름. CHORE-MEAL-TARGET-KCAL-SOT-002 가 후속에서 null 저장 + FE fallback 변경 + DB 컬럼 drop.
...
전체 story 는 `findById`(detail) 에서만 select 한다 — 피드 리스트(`LifestyleClaimWireSchema`)는 full story 미포함이라 payload 가 lean 하다. **단, News 피드 카드 표지(cover)용으로 story.hook 에서 파생한 lean 2필드는 피드에도 포함**한다: `cover_image_url`(= `story->'hook'->>'image'`) + `cover_hook`(= `story->'hook'->>'headline'`). 둘 다 nullable·optional — story(또는 hook.image)가 없으면 null → mobile 이 텍스트 fallback 표지(진초록 + 이니셜 워터마크)를 렌더한다. 풀 story 페이로드를 끌어오지 않고 표지 사진+훅만으로 피드를 렌더하기 위함이다 (IMPL-MOBILE-NEWS-COVER-001). 상세 계약은 `LifestyleClaimDetailWireSchema = LifestyleClaimWireSchema.extend({ story: ClaimStorySchema.nullable() })` 이며 shape 는 seed `_schema.json` 의 claim.story(additionalProperties:false)와 일치한다. 모든 텍스트는 plain(HTML 금지 — validate-claim-seeds.py 가 leaf 재귀 검사) + `**bold**`/`*accent*` 인라인 마크업(RichText 렌더 — 리터럴 마커 미노출).
...
2. **per-slide layout 존중** — 앱 `ClaimDetailScreen` 이 비-hook 슬라이드를 `layout:'band'` 로 하드코딩하던 v1 제한을 제거하고 **저장된 `layout`(fullbleed/band)을 그대로 렌더**한다(생략 시 역할 기본값 hook=fullbleed·그 외=band). fullbleed 정보 슬라이드(what/science/rescaled/catch)는 텍스트를 onDark(cream) 변형으로 렌더 + 텍스트 영역을 덮는 강한 scrim(상단 0.62→하단 0.9, hook 표지는 바닥 위주 가벼운 scrim 유지)으로 가독을 보장하고, 콘텐츠는 ScrollView(`flexGrow:1`+`flex-end`)로 짧으면 바닥 정렬·넘치면 스크롤. `layout` 은 이미 schema/스튜디오/저장/카드템플릿이 지원했으므로 본 변경은 **순수 앱 변경**이다.
...
## 10A. Lean Launch Plan (D-Day 2026-06-18) *(STRATEGY-PROD-LAUNCH-ROADMAP-001)*

> v1 추가 2026-05-14. 본 섹션은 PIVOT-MOBILE banner (§ 상단) 와 일관. §10 Phase 1 의 진행 상태를 prod 출시 트랙으로 정형화. **§10 의 Phase 1~4 product feature roadmap 은 그대로 유효** — 본 §10A 는 출시 운영 트랙의 narrowly additive layer.

### Strategic Decisions (사용자 확정)

| Decision | 답 |
|----------|-----|
| **A. PHI 정책** | medical_conditions + medications launch v1 에서 제거 (post-BAA 후 재도입). allergies + activity_level + body metrics 만 유지 (비-PHI) |
| **B. DB 위치** | EC2 docker Postgres + S3 daily backup. stop conditions 도달 시 RDS 즉시 promotion |
| **C. Timeline** | 5주 aggressive (D-Day 2026-06-18) |
| **D. Plan 구조** | 4 Gate (Account/IAP, PHI/Privacy, Prod Infra, Store Review). D-Day = 4 Gate 통과 조건 |

### 4 Gate 요약

- **G1 Account & IAP Gate**: Apple Developer + Google Play 가입 + AWS SES production access + RevenueCat live (7 sub-gate) + Paid Apps Agreement
- **G2 PHI/Privacy Gate**: medical_conditions/medications field 제거 + selection-based onboarding UX + Privacy Policy + ToS + Apple App Privacy mapping + Sentry PHI redaction + Compliance Decision Record
- **G3 Prod Infra Gate**: prod EC2 + 5 BE 배포 (CD matrix expansion, 7 sub-task) + Cognito prod pool (4 sub-task) + Migration auto-runner + DB backup + Sentry + Capacity budget
- **G4 Store Review Gate**: EAS owner transfer + prod build + store metadata + closed beta (10-20명, pre-review gate) + submission + review pass

### Track 2 — Post-Launch Incremental (deferred)

§9.2 의 50K MAU + ECS Fargate + read replica + PgBouncer 는 본 lean launch 의 **temporary deviation**. promotion threshold 도달 시 §9.2 target 으로 migration. 임계값: p99 latency > 800ms, DB > 5GB, EC2 memory > 80%, 사용자 > 1000, 매출 > $5K/월 등.

### Cross-ref

- `docs/PROD-DEPLOY-ROADMAP.md` — 전체 plan (4 Gate sub-task / 14-finding Risk Register / promotion rules)
- `docs/runbooks/COMPLIANCE-LAUNCH-V1.md` — Compliance Decision Record (HIPAA / FTC HBNR / CCPA / GDPR / vendor list / breach response owner) — G2 시점에 신설
- `docs/IMPLEMENTATION_LOG.md` — 본 task `STRATEGY-PROD-LAUNCH-ROADMAP-001` entry + 4 Gate sub-task 진행 추적
