...
## ✅ Operator Security Checklist (run before apply)

- [ ] AWS credentials belong to a **least-privilege IAM role** scoped to the permissions above — not an admin/root key.
- [ ] **CloudTrail is enabled** in the target region (`us-east-1` + `us-west-2`). Cognito control-plane calls must be auditable.
- [ ] No plaintext secrets in shell history. Use `fileb://` with AWS CLI and **`rm -P secret.json`** (APFS-safe) after upload.
- [ ] `terraform.tfstate` and `.terraform/` are **never committed** to git (already in `.gitignore` — verify before `git add`).
- [ ] Single-operator lock sentinel applied (see ⚠️ section above).

> CHORE-007 will add automated IAM policy enforcement + CloudTrail metric filter alerts.
