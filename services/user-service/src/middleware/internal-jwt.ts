...
const INTERNAL_AUDIENCE = 'user-service:internal';

// In-memory jti replay cache: jti → expires timestamp
...
function markJtiSeen(jti: string): void {
  ...
  setTimeout(() => { jtiCache.delete(jti); }, JTI_TTL_MS);
}
  ...
  app.addHook(
    ...
    async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
      ...
      if (!auth || !auth.startsWith('Bearer ')) {
        ...
        await reply.status(401).send({ error: { code: 'UNAUTHORIZED', message: 'Missing internal token', requestId: request.id } });
        return;
      }
      ...
      try {
        ...
        if (!jti) {
          ...
          await reply.status(401).send({ error: { code: 'UNAUTHORIZED', message: 'Missing jti', requestId: request.id } });
          return;
        }

        if (isJtiSeen(jti)) {
          request.log.warn({ reason: 'jti_replay', path: urlPath }, 'internal_jwt.rejected');
          await reply.status(401).send({ error: { code: 'UNAUTHORIZED', message: 'Token replay detected', requestId: request.id } });
          return;
        }
        ...
        await reply.status(401).send({ error: { code: 'UNAUTHORIZED', message: 'Invalid internal token', requestId: request.id } });
      }
    },
  );
}
