...
### L3 review 의 adversarial fallback (IMPL-AI-002 교훈)

L3 tier 는 nominally "Codex 2 + Gemini 1 (adversarial)" 이지만 CLI 도구 제약으로 실행 불가 시 **Claude self-adversarial pass** 로 대체한다:

- **Codex CLI (gpt-5-codex) review file traversal 함정**: HANDOFF 의 변경 범위가 5+ files 또는 1000+ insertions 일 때 codex CLI 가 review 단계에서 파일 traversal 만 수행하고 80KB+ 출력만 내며 구조화된 finding 을 생성하지 못함 (IMPL-AI-002 r1 에서 두 번 시도 모두 동일 패턴). 해결:
  - file 1~3개 단위 sub-task 로 쪼개거나
  - `.claude/rules/pipeline.md` "Claude is the final judge" 원칙에 따라 Claude 가 직접 `origin/main..HEAD` diff 인스펙트
- **Gemini CLI 0.39.1 도구 부재**: gemini 0.39.1 에는 `run_shell_command` / `Bash` 도구가 없고 `read_file` / `grep_search` / `invoke_agent` 만 존재. → adversarial reviewer 로 동작 불가. 해결: Claude self-adversarial (별도 관점으로 직접 인스펙트) 1 회 추가

판정 절차 (fallback 시):
1. `pipeline-log.jsonl` 에 `review_method: claude_direct` + `reason: codex_traversal_failure` 기록
2. Claude 가 review 1 회 + adversarial pass 1 회 (별도 관점, 예: "공격자가 이 코드로 무엇을 할 수 있나?") 직접 수행
3. review-r{n}.md 에 `> 메모: codex/gemini CLI 한계로 Claude 직접 판정` 명시
4. finding 항목별로 PASS / HIGH / MEDIUM 분류 + fix 후 r{n+1} 재검증

### codex / gemini CLI 정상 동작 케이스 (IMPL-021 교훈, IMPL-AI-002 와 대비 데이터 포인트)

IMPL-AI-002 r1 의 codex CLI traversal 실패는 변경 범위가 컸기 때문 (5+ files / 1000+ insertions). IMPL-021 의 review-r1/r2/r3 에서는 codex + gemini CLI 가 모두 정상으로 structured finding 을 생성. 정상 동작 조건:

- 변경 범위 단일 서비스 (`services/<single-svc>/**`) + 추가 file ≤ 5
- diff 가 origin/main 기준 < 1000 insertions
...
| config 파일 + E2E spec (`.env`, `docker-compose`, `.spec.ts`) | **Claude 직접** | 비즈니스 로직/JSX 없음 — static grep QA로 충분 (IMPL-APP-005-d 교훈) |
